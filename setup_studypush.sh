#!/bin/bash
# ═══════════════════════════════════════════════════════════════
#  StudyPush — One-Click Setup Script
#  Run this ONCE to set up daily auto-push to your GitHub repo.
#
#  Usage:
#    chmod +x setup_studypush.sh
#    ./setup_studypush.sh
#
#  Prerequisites:
#    - git installed and authenticated (SSH or HTTPS)
#    - Your GitHub repo already created (can be empty)
# ═══════════════════════════════════════════════════════════════

set -e

# ── Configuration (EDIT THESE) ────────────────────────────────
GITHUB_USER="akashrane"
REPO_NAME="StudyPush"
BRANCH="main"
SCHEDULE_HOUR=9  # UTC hour (9 UTC = 5AM EST = 2:30PM IST)
# ──────────────────────────────────────────────────────────────

echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║   📚 StudyPush — One-Click Setup             ║"
echo "║   Daily Data Science Articles on GitHub       ║"
echo "╚══════════════════════════════════════════════╝"
echo ""
...
if [ -d "$REPO_NAME" ]; then
    ...
    git clone "$REPO_URL" && cd "$REPO_NAME" || {
        echo "   → Repo might be empty. Initializing fresh..."
...
echo ""
echo "⚙️  Step 2/5 — Creating GitHub Actions workflow..."
mkdir -p .github/workflows

cat > .github/workflows/daily-push.yml << 'WORKFLOW_EOF'
name: "\U0001F4DA Daily Data Science Article Push"
...
jobs:
  generate-and-push:
    ...
    steps:
      ...
      - name: "\U0001F4DD Generate random article"
        run: python generate_article.py

      - name: "\U0001F680 Commit and push"
        run: |
          git config --local user.email "github-actions[bot]@users.noreply.github.com"
          git config --local user.name "StudyPush Bot \U0001F916"
          git add -A
          DATE=$(date +'%Y-%m-%d')
          git diff --cached --quiet || git commit -m "\U0001F4D8 Daily DS Article — ${DATE}" -m "Auto-generated data science article for daily study push."
          git push
WORKFLOW_EOF

echo "   ✅ Workflow created"

# Step 3: Create the article generator
echo ""
echo "🐍 Step 3/5 — Creating article generator (20+ DS topics)..."

cat > generate_article.py << 'PYTHON_EOF'
"""
...
echo "   ✅ Article generator created (20+ topics)"

# Step 4: Create README
echo ""
echo "📝 Step 4/5 — Creating README..."
...
# Generate the first article
python3 generate_article.py

git add -A
git commit -m "🚀 Setup StudyPush — daily data science auto-push" \
          -m "Added GitHub Actions workflow, article generator (20+ DS topics), and README."
...
echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║   ✅  ALL DONE!  StudyPush is live!           ║"
echo "╠══════════════════════════════════════════════╣"
echo "║                                              ║"
echo "║  📅 Schedule: Every day at ${SCHEDULE_HOUR}:00 UTC         ║"
...
echo "║  → Edit .github/workflows/daily-push.yml     ║"
echo "║                                              ║"
echo "╚══════════════════════════════════════════════╝"
echo ""